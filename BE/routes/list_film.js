...
router.get('/list_film/status/:matt', async function(req, res) {
    let matt = req.params.matt;

    try {
        let [phimList] = await connection.query(
            'SELECT MAPHIM, TENPHIM, HINH_ANH_URL FROM PHIM WHERE MATT = ? ORDER BY CAST(SUBSTRING(MAPHIM, 2) AS UNSIGNED)',
            [matt]
        );
